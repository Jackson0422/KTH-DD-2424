...
def initial_net(L, d, m):  # need the number of layers, dimension and size of layer
    ...
    net_params['W'][0] = rng.standard_normal(size=(m, 3072)) / np.sqrt(d)
    net_params['W'][1] = rng.standard_normal(size=(10, m)) / np.sqrt(m)
...
def ApplyNetwork(X, net_params, dropoup_p = 0.5, Train = True):
    fp_data = {}
    ...
    # Exercise 5.1 (b)
    if Train:
        keep_drop = 1 - dropoup_p
        dropout_mask = (np.random.rand(*h.shape) > dropoup_p).astype(float)
        h *= dropout_mask
        h /= keep_drop
...
def Backwardpass(X, Y, P, net_params, fp_data, lam, dropout_p = 0.5, Train = True):
    grads = {}
    grads['W'] = [None] * 2
    grads['b'] = [None] * 2
    N = X.shape[1]
    ...
    if Train:
        ...
        dropout_mask = fp_data['dropout_mask']
        G_hidded *= dropout_mask
        G_hidded /= keep_prob
...
def MiniBatchGD(X, Y, GDparams, init_net, lam, X_val, Y_val):
    ...
    for i in tqdm.tqdm(range(GDparams['n_epochs']), desc="Epoch Running"):
        ...
        for j in range(n // GDparams['n_batch']):
            ...
            Xbatch_after = data_augmentation(tx, ty, Xbatch)
            ...
            if t_mod <= GDparams['n_s']:
                ...
                eta_t = GDparams['eta_max'] - ((t_mod - GDparams['n_s']) / GDparams['n_s']) * (
                            GDparams['eta_max'] - GDparams['eta_min'])
            lr_list.append(eta_t)
            P_1, fp_data_1 = ApplyNetwork(Xbatch_after, trained_net, 0.5, True)
            grads_my = Backwardpass(Xbatch_after, Ybatch, P_1, trained_net, fp_data_1, lam, 0.5, True)
            for k in range(len(trained_net['W'])):
                trained_net['W'][k] -= eta_t * grads_my['W'][k]
                trained_net['b'][k] -= eta_t * grads_my['b'][k]
            t += 1
            P_2, fp_data_2 = ApplyNetwork(X, trained_net, 0.5, False)
        ...
        P_3, fp_data_3 = ApplyNetwork(X_val, trained_net, 0.5, False)
    ...
    return trained_net, train_loss_list, train_cost_list, train_accuracy_list, val_loss_list, val_cost_list, val_accuracy_list, lr_list
...
def coarse_search_lam(X_train, Y_train, X_val, Y_val, lam_min, lam_max, num_lam):
    ...
    for lam in tqdm.tqdm(lam_candidates, desc="Searching lambda candidates"):
        ...
        trained_net, train_loss_list, train_cost_list, train_accuracy_list, val_loss_list, val_cost_list, val_accuracy_list, lr_list = MiniBatchGD(
            X_train, Y_train, GDparams, init_net, lam, X_val, Y_val)
...
def plot_show(img, title="Image", mean=None, std=None):
...
def init_net_Adam(L, d, m):
    ...
    # Adam
    net_params['mW'] = [None] * L
    net_params['vW'] = [None] * L
    net_params['mb'] = [None] * L
    net_params['vb'] = [None] * L
    ...
    # Adam
    net_params['mW'][0] = np.zeros_like(net_params['W'][0])
    net_params['mW'][1] = np.zeros_like(net_params['W'][1])
    net_params['vW'][0] = np.zeros_like(net_params['W'][0])
    net_params['vW'][1] = np.zeros_like(net_params['W'][1])

    net_params['mb'][0] = np.zeros_like(net_params['b'][0])
    net_params['mb'][1] = np.zeros_like(net_params['b'][1])
    net_params['vb'][0] = np.zeros_like(net_params['b'][0])
    net_params['vb'][1] = np.zeros_like(net_params['b'][1])

    return net_params

def MiniBatchGD_Adam(X, Y, GDparams, init_net, lam, X_val, Y_val):
    LR = GDparams['LearningRate']
    ...
    n = X.shape[1]
    t = 0
    for epoch in tqdm.tqdm(range(n_epochs), desc="Epoch Running"):
        permutation = np.random.permutation(n)
        for j in range(n // n_batch):
            j_start = j * n_batch
            j_end = (j + 1) * n_batch
            inds = permutation[j_start:j_end]
            Xbatch = X[:, inds]
            Ybatch = Y[:, inds]
            ...
            P_1, fp_data = ApplyNetwork(Xbatch, trained_net, dropoup_p=GDparams['dropout_p'], Train=True)
            grads = Backwardpass(Xbatch, Ybatch, P_1, trained_net, fp_data, lam, dropout_p=GDparams['dropout_p'],Train=True)
            t += 1

            for k in range(len(trained_net['W'])):
                trained_net['mW'][k] = beta1 * trained_net['mW'][k] + (1 - beta1) * grads['W'][k]
                trained_net['mb'][k] = beta1 * trained_net['mb'][k] + (1 - beta1) * grads['b'][k]

                trained_net['vW'][k] = beta2 * trained_net['vW'][k] + (1 - beta2) * (grads['W'][k] ** 2)
                trained_net['vb'][k] = beta2 * trained_net['vb'][k] + (1 - beta2) * (grads['b'][k] ** 2)

                mW_hat = trained_net['mW'][k] / (1 - beta1 ** t)
                mb_hat = trained_net['mb'][k] / (1 - beta1 ** t)
                vW_hat = trained_net['vW'][k] / (1 - beta2 ** t)
                vb_hat = trained_net['vb'][k] / (1 - beta2 ** t)

                trained_net['W'][k] -= LR * mW_hat / (np.sqrt(vW_hat) + epsilon)
                trained_net['b'][k] -= LR * mb_hat / (np.sqrt(vb_hat) + epsilon)

                P_train, fp_data_train = ApplyNetwork(X, trained_net, dropoup_p=GDparams['dropout_p'], Train=False)
                cost_train, loss_train = cost_function(P_train, Y, X, lam, trained_net)
                acc_train = ComputeAccuracy(P_train, Y)
                train_cost_list.append(cost_train)
                train_loss_list.append(loss_train)
                train_accuracy_list.append(acc_train)

        P_val, fp_data_val = ApplyNetwork(X_val, trained_net, dropoup_p=GDparams['dropout_p'], Train=False)
        cost_val, loss_val = cost_function(P_val, Y_val, X_val, lam, trained_net)
        acc_val = ComputeAccuracy(P_val, Y_val)
        val_cost_list.append(cost_val)
        val_loss_list.append(loss_val)
        val_accuracy_list.append(acc_val)

    return trained_net, train_loss_list, train_cost_list, train_accuracy_list, val_loss_list, val_cost_list, val_accuracy_list
...
def data_augmentation(tx, ty, X, p = 0.5):
    ...
    inds_flip = np.vstack((ind_flip, 1024 + ind_flip, 2048 + ind_flip)).squeeze()

    if tx >= 0:
        w_col = 32 - tx
        col_fill = np.arange(tx, 32)
        col_src = np.arange(0, 32 - tx)
    else:
        shift_left = -tx
        w_col = 32 - shift_left
        col_fill = np.arange(0, 32 - shift_left)
        col_src = np.arange(shift_left, 32)
    aa = np.arange(32).reshape((32, 1))
    vv = np.tile(32 * aa, (1, w_col))

    bb1 = col_fill.reshape((w_col, 1))
    bb2 = col_src.reshape((w_col, 1))

    ind_fill_temp = vv.reshape((32 * w_col, 1)) + np.tile(bb1, (32, 1))
    ind_xx_temp = vv.reshape((32 * w_col, 1)) + np.tile(bb2, (32, 1))

    if ty > 0:
        ii = np.transpose(np.nonzero(ind_fill_temp >= ty * 32))
        if len(ii) == 0:
            return X_aug
        start_id = ii[0, 0]
        ind_fill_temp = ind_fill_temp[start_id:]
        ind_xx_temp = ind_xx_temp[start_id:]

        ii = np.transpose(np.nonzero(ind_xx_temp < (1024 - ty * 32)))
        if len(ii) == 0:
            return X_aug
        end_id = ii[-1, 0]
        ind_xx_temp = ind_xx_temp[:end_id + 1]
        ind_fill_temp = ind_fill_temp[:end_id + 1]

    else:
        row_shift = -ty
        limit_high = 1024 + ty * 32
        ii = np.transpose(np.nonzero(ind_fill_temp < limit_high))
        if len(ii) == 0:
            return X_aug
        max_id = ii[-1, 0]
        ind_fill_temp = ind_fill_temp[:max_id + 1]
        ind_xx_temp = ind_xx_temp[:max_id + 1]
    inds_fill = np.vstack((
        ind_fill_temp,
        1024 + ind_fill_temp,
        2048 + ind_fill_temp
    ...
    for i in range(N):
        if np.random.rand() < p:
            original_img = X[:, i]
            #plot_show(original_img, title=f"Original Image {i}", mean=mean, std=std)

            X_aug[:, i] = X_aug[inds_flip, i]
            mask_valid = (inds_fill < 3072) & (inds_xx < 3072)
            valid_ids = np.where(mask_valid)[0]
            use_fill = inds_fill[valid_ids]
            use_xx = inds_xx[valid_ids]
            X_aug[use_fill, i] = X_aug[:, i][use_xx]

            # plot_show(X_aug[:, i], title=f"Augmented Image {i}", mean=mean, std=std)
    return X_aug
...
if __name__ == '__main__':
    ...
    all_data_label_onehot = np.hstack((batch1_label_onehot, batch2_label_onehot, batch3_label_onehot,
                                       batch4_label_onehot, batch5_label_onehot))  # 10 * 50000
    ...
    train_data_after = normalize(mean_train_data, std_train_data,
                                 train_data)  # normalize the training data            3027 * 45000
    ...
    # GDparams['n_epochs'] = int(update_step / (n // GDparams['n_batch']))
    #
    # HiddenNode_candidate_list = [100, 200, 300]
    # lam_candidate_list = [1e-4, 1e-3, 1e-2]
    # Accuracy_list = []
    # for HiddenNode in tqdm.tqdm(HiddenNode_candidate_list, desc="Hidden nodes"):
    #     for lam in tqdm.tqdm(lam_candidate_list, desc="Lambda"):
    #         init_net = initial_net(L=2, d=3072, m=HiddenNode)
    #         trained_net, train_loss_list, train_cost_list, train_accuracy_list, val_loss_list, val_cost_list, val_accuracy_list, lr_list = MiniBatchGD(
    #             train_data_after,
    #             train_onehot, GDparams,
    #             init_net, lam, val_data_after, val_onehot)
    #         P_test, fapa_data = ApplyNetwork(test_data_after, trained_net, 0.5, False)
    ...
    #         plt.legend(loc='upper right')
    #         plt.show()
    # print("Accuracy on test set:", Accuracy_list)

    # Exercise 5.1 (b + c test)
    # GDparams = {}
    # GDparams['n_batch'] = 100
    # GDparams['eta_min'] = 1e-5
    # GDparams['eta_max'] = 1e-1
    ...
    # update_step = 6 * GDparams['n_s']
    # GDparams['n_epochs'] = int(update_step / (n // GDparams['n_batch']))
    # init_net = initial_net(L=2, d=3072, m=300)
    # lam = 1e-3
    # trained_net, train_loss_list, train_cost_list, train_accuracy_list, val_loss_list, val_cost_list, val_accuracy_list, lr_list = MiniBatchGD(
    #         train_data_after,
    #         train_onehot, GDparams,
    ...
    # plt.legend(loc='upper right')
    # plt.show()
    # print("The accuracy on test is: ", Accuracy)
    ...
    # n = train_data_after.shape[1]
    # GDparams['n_s'] = 2 * np.floor(n / GDparams['n_batch'])
    # update_step = 8 * GDparams['n_s']
    # GDparams['n_epochs'] = int(update_step / (n // GDparams['n_batch']))
    #
    # HiddenNode_candidate_list = [200, 300]
    # lam_candidate_list = [1e-4, 1e-3, 1e-2]
    # Accuracy_list = []
    # for HiddenNode in tqdm.tqdm(HiddenNode_candidate_list, desc="Hidden nodes"):
    #     for lam in tqdm.tqdm(lam_candidate_list, desc="Lambda"):
    ...
    # best_setting = {}
    #
    # for lr in tqdm.tqdm(lr_list, desc="Learning rate"):
    #     for b1 in tqdm.tqdm(beta1_list, desc="Beta1"):
    #         for b2 in tqdm.tqdm(beta2_list, desc="Beta2"):
    #             GDparams = {
    #                 'n_batch': 100,
    #                 'n_epochs': 4,
    #                 'LearningRate': lr,
    ...
    #             }
    #
    #             init_net = init_net_Adam(L=2, d=3072, m=300)
    #             lam = 1e-3
    #
    #             trained_net, train_loss_list, train_cost_list, train_accuracy_list, val_loss_list, val_cost_list, val_accuracy_list = MiniBatchGD_Adam(
    #                 train_data_after, train_onehot,
    #                 GDparams, init_net, lam, val_data_after, val_onehot)
    #
    #             P_test, fapa_data = ApplyNetwork(test_data_after, trained_net, dropoup_p=GDparams['dropout_p'], Train=False)
    #             acc = ComputeAccuracy(P_test, test1_label_onehot)
    #             acc_list.append(acc)
    #
    ...
    #             if acc > best_acc:
    #                 best_acc = acc
    #                 best_setting = {'lr': lr, 'beta1': b1, 'beta2': b2}
    #
    # print("\n=== Best Adam setting ===")
    # print("Accuracy list is: ", acc_list)
    # print(f"lr = {best_setting['lr']}")
    # print(f"beta1 = {best_setting['beta1']}")
    # print(f"beta2 = {best_setting['beta2']}")
    # print(f"Best Test Accuracy = {best_acc:.4f}")

    GDparams = {}
    GDparams['n_batch'] = 100
    GDparams['n_epochs'] = 15
    GDparams['LearningRate'] = 3e-4
    GDparams['beta1'] = 0.95
    GDparams['beta2'] = 0.99
    GDparams['epsilon'] = 1e-8
    GDparams['dropout_p'] = 0.5
