...
def MiniBatch_GD(X_ims, Y, MX, GDparams, init_net, Y_val, MX_val):
    ...
    for i in tqdm.tqdm(range(n_epoch), desc="Epoch Running",  dynamic_ncols=True):
        ...
        for j in tqdm.tqdm(range(n // n_batch), desc="Batch Running",  dynamic_ncols=True):
            ...
            MX_batch = MX_train[:, :, j_start:j_end]
            Y_batch = Y_train[:, j_start:j_end]

            # forward and backward
            p_batch, h_batch,x1_batch = forward_pass(MX_batch, net_params)
            grads = back_pass(MX_batch, Y_batch, net_params, h_batch, x1_batch, p_batch,16, 40, lam)

            # CLR learning rate
            cycle = int(np.floor(t / (2 * current_n_s)))
            t_mod = t - 2 *  current_n_s * cycle
            if t_mod <=  current_n_s:
                eta_t = min_eta + (t_mod /  current_n_s) * (max_eta - min_eta)
            else:
                eta_t = max_eta - ((t_mod -  current_n_s) /  current_n_s) * (max_eta - min_eta)
...
if __name__ == '__main__':
    ...
    # print("F size is:", Fs.shape)
    # X_ims = np.transpose(X.reshape((32, 32, 3, n), order='F'), (1, 0, 2, 3))
    # print("X_ims shape is:", X_ims.shape)
    #
    # conv_res = conv_res(X_ims, Fs)
    # print("conv_res shape is:", conv_res.shape)
    # conv_output_debug = load_data['conv_outputs']
    # print("conv_res_debug shape is:", conv_output_debug.shape)
    #
    # f_filters_size = Fs.shape[0]
    # MX =MX_C(X_ims, f_filters_size)
    # Fs_flat = Fs.reshape(4*4*3,2)
    # conv_res_MX = conv_MX(MX, Fs_flat)
    # print("Use MX get conv result is:", conv_res_MX.shape)
   ...
   #  n = X.shape[-1]  # 5
   #  X_ims = np.transpose(X.reshape((32, 32, 3, n), order='F'), (1, 0, 2, 3))
   #  net_params = initialize_parameters(4, 2, 10, 64)
   #  bf = np.zeros((2, 1))
   #  net_params0 ={
   #      'W1':load_data['W1'],
   #      'b1':load_data['b1'],
   #      'W2':load_data['W2'],
   #      'b2':load_data['b2'],
   #      'Fs_flat':load_data['Fs_flat'],
   #      'b_f':bf
   #  }
   #  MX = MX_C(X_ims, 4)
   #  p,h,x1= forward_pass(MX, net_params0)
   #  grads_npy,grad_s_numpy,p_numpy = back_pass(MX, load_data['Y'], net_params0, h, x1, p,64, 2)
   #  grads_torch,P_torch = ComputeGradsWithTorch(MX, load_data['Y'], net_params0, 4)
   #
   #  s_torch_np = P_torch.detach().cpu().numpy()
   #  max_diff_x1 = np.max(np.abs(p_numpy - s_torch_np))
   #  print("反向传播P的最大差值：", max_diff_x1)

   # # P_torch_np = P_torch.detach().cpu().numpy()
   #  #max_diff_P = np.max(np.abs(p_numpy - P_torch_np))
   #  # print("自己的前向传播和torch的前向传播大小是：", type(conv_out_MX), type(H_torch))
   #  # H_torch_np = H_torch.detach().cpu().numpy()
   #  # max_diff_H = np.max(np.abs(conv_out_MX - H_torch_np))
   #  #print("前向传播P的最大差值：", max_diff_P)
   #  #print("前向传播H的最大差值：", max_diff_H)
   #  # s_torch_np = P_torch.detach().cpu().numpy()
   #  # max_diff_x1 = np.max(np.abs(p - s_torch_np))
   #  # print("前向传播p的最大差值：", max_diff_x1)\
   #
   #  print("=============================================================================================")
   #  grad_W1_debug = load_data['grad_W1']
   #  grad_W2_debug = load_data['grad_W2']
   #  grad_b1_debug = load_data['grad_b1']
   #  grad_b2_debug = load_data['grad_b2']
   #  grad_Fs_debug = load_data['grad_Fs_flat']
   #
   #  grad_W1 = grads_npy['W1']
   #  grad_W2 = grads_npy['W2']
   #  grad_b1 = grads_npy['b1']
   #  grad_b2 = grads_npy['b2']
   #  grad_Fs = grads_npy['filters']
   #
   #  max_diff_W1 = np.max(np.abs(grad_W1 - grad_W1_debug))
   #  max_diff_W2 = np.max(np.abs(grad_W2 - grad_W2_debug))
   #  max_diff_b1 = np.max(np.abs(grad_b1 - grad_b1_debug))
   #  max_diff_b2 = np.max(np.abs(grad_b2 - grad_b2_debug))
   #  max_diff_Fs = np.max(np.abs(grad_Fs - grad_Fs_debug))
   #  print("使用官方debug文件算W1,W2,b1,b2,max_diff_Fs差值的是:", max_diff_W1,max_diff_W2,max_diff_b1,max_diff_b2,max_diff_Fs )
